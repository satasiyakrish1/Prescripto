...
export const getPosts = async (req, res) => {
    try {
        ...
        // For each post, add the user's posting permission status
        const postsWithPermissions = await Promise.all(posts.map(async (post) => {
            const user = await userModel.findById(post.userId);
            return {
                ...post.toObject(),
                userCanPost: user ? user.canPostInCommunity : false
            };
        }));

        res.json({ success: true, posts: postsWithPermissions });
...
export const deletePost = async (req, res) => {
    try {
        ...
        const deletedPost = await communityPostModel.findByIdAndDelete(postId);
        if (!deletedPost) {
...
// Toggle user's posting permission (Admin only)
export const togglePostingPermission = async (req, res) => {
    try {
        ...
        user.canPostInCommunity = permission;
        await user.save();
