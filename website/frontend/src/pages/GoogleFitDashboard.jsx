...
const GoogleFitDashboard = () => {
  ...
  const connectGoogleFit = async () => {
    console.log('Debug - connectGoogleFit called');
    console.log('Debug - backendUrl:', backendUrl);
    console.log('Debug - token exists:', !!token);
    ...
    try {
      console.log('Debug - Making request to:', `${backendUrl}/api/user/google-fit/auth-url`);
  ...
  const getFitnessData = async () => {
    try {
      setIsLoading(true);
      console.log('Debug - Fetching fitness data from:', `${backendUrl}/api/user/google-fit/data`);
      const { data } = await axios.get(`${backendUrl}/api/user/google-fit/data`, {
        headers: { token }
      });
      console.log('Debug - Fitness data response:', data);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8 flex justify-between items-center">
        ...
        {isConnected && (
          <a
            href="/fitness-analysis"
            className="bg-primary text-white px-4 py-2 rounded-full text-sm hover:bg-opacity-90 transition-colors"
          >
            📊 Detailed Analysis
          </a>
        )}
